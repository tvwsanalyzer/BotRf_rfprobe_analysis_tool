...
{	double A_ref;      // reference attenuation
	...
	double rch[2];
	double wn;          // wave number (= radio frequency)
	...
	double the[2];		// horizontal elevation angles
	double tiw;
	double ght;
	double ghr;
	double rph;
...
double saalos(double d, prop_type &prop, propa_type &propa)
{
	...
	hone=prop.tgh+prop.tsgh-(prop.rch[1]-prop.h_g[1]);  
				
		if(prop.tgh>prop.cch)  /* for TX ant above all clutter height*/	
		{
			ensa=1+prop.N_s*0.000001;	
			...
			else
			{
				...
				arte=q+(0.7*pdk-mymax(0.01,log10(prop.wn*47.7)-2))*(prop.h_g[1]/hone);
				}
		...
		else  /* for TX at or below clutter height */
		{
			...
			arte=arte-(mymax(0.01,log10(prop.wn*47.7)-2))*(prop.h_g[1]/prop.tgh);
		}
		saalosv=arte;
	}
	return saalosv;
...
double adiff(double d, prop_type &prop, propa_type &propa)
{
	...
	if (d==0)
	{
		...
		wd1=sqrt(1.0+qk/q);
		xd1=propa.dla+propa.tha/prop.gamma_e;
		q=(1.0-0.8*exp(-propa.d_Ls/50e3))*prop.delta_h;
		q*=0.78*exp(-pow(q/16.0,0.25));
		afo=mymin(15.0,2.171*log(1.0+4.77e-4*prop.h_g[0]*prop.h_g[1]*prop.wn*q));
		...
		for (int j=0; j<2; ++j)
		{
			/* a=0.5*pow(prop.d_Lj[j],2.0)/prop.h_e[j]; */
			a=0.5*(prop.d_Lj[j]*prop.d_Lj[j])/prop.h_e[j];
			wa=pow(a*prop.wn,THIRD);
			pk=qk/wa;
			q=(1.607-pk)*151.0*wa*prop.d_Lj[j]/a;
			xht+=q;
			aht+=fht(q,pk);
		}
	...
	{
		th=propa.tha+d*prop.gamma_e;
		ds=d-propa.dla;
		/* q=0.0795775*prop.wn*ds*pow(th,2.0); */
		q=0.0795775*prop.wn*ds*th*th;
		adiffv=aknfe(q*prop.d_Lj[0]/(ds+prop.d_Lj[0]))+aknfe(q*prop.d_Lj[1]/(ds+prop.d_Lj[1]));
		a=ds/th;
		wa=pow(a*prop.wn,THIRD);
		pk=qk/wa;
		q=(1.607-pk)*151.0*wa*th+xht;
		ar=0.05751*q-4.343*log(q)-aht;
		q=(wd1+xd1/d)*mymin(((1.0-0.8*exp(-d/50e3))*prop.delta_h*prop.wn),6283.2);
		wd=25.1/(25.1+sqrt(q));
...
double adiff2(double d, prop_type &prop, propa_type &propa)
{
	...
	if (d==0)
	{
		...
		aht=20.0;
		xht=0.0;
		a=0.5*(prop.d_Lj[0]*prop.d_Lj[0])/prop.h_e[0];
		wa=pow(a*prop.wn,THIRD);
		pk=qk/wa;
		q=(1.607-pk)*151.0*wa*prop.d_Lj[0]/a;
		xht=q;
		aht+=fht(q,pk);
						

		if ((int(prop.d_Lj[1])==0.0) || (prop.the[1]>0.2))
		{ 
			xht+=xht;
			aht+=(aht-20.0);
		}
		...
		{
			a=0.5*(prop.d_Lj[1]*prop.d_Lj[1])/prop.h_e[1];
			wa=pow(a*prop.wn,THIRD);
			pk=qk/wa;
			q=(1.607-pk)*151.0*wa*prop.d_Lj[1]/a;
			xht+=q;
			aht+=fht(q,pk);
		}
		adiffv2=0.0;
 	...
 	else
	{
		...
		wa=pow(a*prop.wn,THIRD);
		pk=qk/wa;
		toh=prop.hht-(prop.rch[0]-prop.d_Lj[0]*((prop.rch[1]-prop.rch[0])/prop.dist));		
		roh=prop.hhr-(prop.rch[0]-(prop.dist-prop.d_Lj[1])*((prop.rch[1]-prop.rch[0])/prop.dist));		
		toho=prop.hht-(prop.rch[0]-(prop.d_Lj[0]+dsl)*((prop.hhr-prop.rch[0])/(prop.dist-prop.d_Lj[1])));		
		roho=prop.hhr-(prop.hht-dsl*((prop.rch[1]-prop.hht)/dsl));	
		dto=sqrt(prop.d_Lj[0]*prop.d_Lj[0]+toh*toh);		
		dto+=prop.gamma_e*prop.d_Lj[0];
		dto1=sqrt(prop.d_Lj[0]*prop.d_Lj[0]+toho*toho);		
		dto1+=prop.gamma_e*prop.d_Lj[0];
		dtro=sqrt((prop.d_Lj[0]+dsl)*(prop.d_Lj[0]+dsl)+prop.hhr*prop.hhr);
		dtro+=prop.gamma_e*(prop.d_Lj[0]+dsl);
		drto=sqrt((prop.d_Lj[1]+dsl)*(prop.d_Lj[1]+dsl)+prop.hht*prop.hht);
		drto+=prop.gamma_e*(prop.d_Lj[1]+dsl);
		dro=sqrt(prop.d_Lj[1]*prop.d_Lj[1]+roh*roh);
		dro+=prop.gamma_e*(prop.d_Lj[1]);
		dro2=sqrt(prop.d_Lj[1]*prop.d_Lj[1]+roho*roho);
		dro2+=prop.gamma_e*(prop.d_Lj[1]);
		dtr=sqrt(prop.dist*prop.dist+(prop.rch[0]-prop.rch[1])*(prop.rch[0]-prop.rch[1]));
		dtr+=prop.gamma_e*prop.dist;
		dhh1=sqrt((prop.dist-propa.dla)*(prop.dist-propa.dla)+toho*toho);
		dhh1+=prop.gamma_e*(prop.dist-propa.dla);
		dhh2=sqrt((prop.dist-propa.dla)*(prop.dist-propa.dla)+roho*roho);
		dhh2+=prop.gamma_e*(prop.dist-propa.dla);

		/* for 1 obst tree base path */
		dtof=sqrt(prop.d_Lj[0]*prop.d_Lj[0]+(toh-prop.cch)*(toh-prop.cch));		
		dtof+=prop.gamma_e*prop.d_Lj[0];
		dto1f=sqrt(prop.d_Lj[0]*prop.d_Lj[0]+(toho-prop.cch)*(toho-prop.cch));		
		dto1f+=prop.gamma_e*prop.d_Lj[0];
		drof=sqrt(prop.d_Lj[1]*prop.d_Lj[1]+(roh-prop.cch)*(roh-prop.cch));
		drof+=prop.gamma_e*(prop.d_Lj[1]);
		dro2f=sqrt(prop.d_Lj[1]*prop.d_Lj[1]+(roho-prop.cch)*(roho-prop.cch));
		dro2f+=prop.gamma_e*(prop.d_Lj[1]);
		...
		{
			if(int(prop.d_Lj[1])>0.0) /* receive site past 2nd peak */
			{
					...
					q=(1.0-0.8*exp(-d/50e3))*prop.delta_h;	
		...
		else /* for single obstacle */
		{
			...
			if(int(prop.d_Lj[1])>0.0) /* receive site past 1st peak */
			{
...
double ascat( double d, prop_type &prop, propa_type &propa)
{
	...
	if (d==0.0)
	{
		...
		etq=(5.67e-6*prop.N_s-2.32e-3)*prop.N_s+0.031;
		h0s=-15.0;
		ascatv=0.0;
	}
	...
	else
	{
		...
		{
			th=prop.the[0]+prop.the[1]+d*prop.gamma_e;
			r2=2.0*prop.wn*th;
			r1=r2*prop.h_e[0];
			r2*=prop.h_e[1];
		...
		h0s=h0;
		th=propa.tha+d*prop.gamma_e;
		/* ascatv=ahd(th*d)+4.343*log(47.7*prop.wn*pow(th,4.0))-0.1*(prop.N_s-301.0)*exp(-th*d/40e3)+h0; */
		ascatv=ahd(th*d)+4.343*log(47.7*prop.wn*(th*th*th*th))-0.1*(prop.N_s-301.0)*exp(-th*d/40e3)+h0;
	}
...
void qlrps(double fmhz, double zsys, double en0, int ipol, double eps, double sgm, prop_type &prop)
{
	...
	if (zsys!=0.0)
		prop.N_s*=exp(-zsys/9460.0);

	prop.gamma_e=gma*(1.0-0.04665*exp(prop.N_s/179.3));
...
double alos(double d, prop_type &prop, propa_type &propa)
{
	...
	if (d==0.0)
	{
		wls=0.021/(0.021+prop.wn*prop.delta_h/mymax(10e3,propa.d_Ls));
		alosv=0.0;
	}
	...
	else
	{
		...
		q=prop.wn*prop.h_e[0]*prop.h_e[1]*2.0/d;
...
double alos2(double d, prop_type &prop, propa_type &propa)
{
	...
	else
	{
		...
		q=(1.0-0.8*exp(-pd/50e3))*prop.delta_h;
		...
		if (prop.mdp<0)
		{
			...
			if ((sps<0.05) && (prop.cch>hrg) && (prop.dist< prop.d_Lj[0])) /* if far from transmitter and receiver below canopy */  
			{
				cd=mymax(0.01,pd*(prop.cch-hrg)/(htg-hrg));
				cr=mymax(0.01,pd-dr+dr*(prop.cch-drh)/htg);
				q=((1.0-0.8*exp(-pd/50e3))*prop.delta_h*(mymin(-20*log10(cd/cr),1.0)));	
			}
		}
		...
		q=prop.wn*prop.h_e[0]*prop.h_e[1]/(pd*3.1415926535897);		
		...
		alosv=-10*log10(re);
		prop.tgh=prop.h_g[0];  /*tx above gnd hgt set to antenna height AGL */	
		prop.tsgh=prop.rch[0]-prop.h_g[0]; /* tsgh set to tx site gl AMSL */		
							
		if ((prop.h_g[1]<prop.cch) && (prop.thera<0.785) && (prop.thenr<0.785))
		{					
			if (sps<0.05)
			{			
...
void qlra(int kst[], int klimx, int mdvarx, prop_type &prop, propv_type &propv)
{
	...
	for (int j=0; j<2; ++j)
	{
		...
		prop.d_Lj[j]=q*exp(-0.07*sqrt(prop.delta_h/mymax(prop.h_e[j],5.0)));
		prop.the[j]=(0.65*prop.delta_h*(q/prop.d_Lj[j]-1.0)-2.0*prop.h_e[j])/q;
	}
...
void lrprop (double d, prop_type &prop, propa_type &propa)
{
	...
	if (prop.mdp!=0)
  	{
		for (j=0; j<2; j++)
			propa.dls[j]=sqrt(2.0*prop.h_e[j]/prop.gamma_e);

		propa.d_Ls=propa.dls[0]+propa.dls[1];
		propa.dla=prop.d_Lj[0]+prop.d_Lj[1];
		propa.tha=mymax(prop.the[0]+prop.the[1],-propa.dla*prop.gamma_e);
		wlos=false;
		wscat=false;
...
// @@@@@@@@ FORSE QUI C'E' L'ERRORE:
// #ifdef ENABLED
		for (j=0; j<2; j++)
		{
			if (abs(prop.the[j]) >200e-3 || prop.d_Lj[j]<0.1*propa.dls[j] || prop.d_Lj[j]>3.0*propa.dls[j] )
			{	
			...
			if(f_fMr==1)
			{
			fprintf(fMr, "lrprop : [j=%i][prop.kwx=%i]\n", j, prop.kwx);
			}
			//-----------------------
#endif			
		}
// #endif
		...
		if (prop.N_s < 250.0 || prop.N_s > 400.0 || prop.gamma_e < 75e-9 || prop.gamma_e > 250e-9 || prop_zgnd.real() <= abs(prop_zgnd.imag()) || prop.wn < 0.419 || prop.wn > 420.0)
			prop.kwx=4;
		...
		/* xae=pow(prop.wn*pow(prop.gamma_e,2.),-THIRD); -- JDM made argument 2 a double */
		xae=pow(prop.wn*(prop.gamma_e*prop.gamma_e),-THIRD);  /* No 2nd pow() */
		d3=mymax(propa.d_Ls,1.3787*xae+propa.dla);
		d4=d3+2.7574*xae;
	...
	if (prop.dist<propa.d_Ls)
	{
		if (!wlos)
		{
			...
			d0=1.908*prop.wn*prop.h_e[0]*prop.h_e[1];
		...
		if (prop.dist>0.0)
		{
			prop.A_ref=propa.ael+propa.ak1*prop.dist+propa.ak2*log(prop.dist);
		
#ifdef DBG_ITWOM
			//-----------------------
			if(f_fMr==1)
			{
			fprintf(fMr, "lrprop : (prop.dist>0.0)[prop.A_ref=%lf]\n", prop.A_ref);
			}
			//-----------------------
#endif			
		}
	...
	if (prop.dist<=0.0 || prop.dist>=propa.d_Ls)
	{
		if(!wscat)
		{ 
			...
			if (a5<1000.0)
			{
				...
				propa.dx=mymax(propa.d_Ls,mymax(propa.dla+0.3*xae*log(47.7*prop.wn),(a5-propa.aed-propa.ems*d5)/(propa.emd-propa.ems)));
		...
		{
			prop.A_ref=propa.aes+propa.ems*prop.dist;
		
#ifdef DBG_ITWOM
			//-----------------------
			if(f_fMr==1)
			{
			fprintf(fMr, "lrprop : (prop.dist>propa.dx)[prop.A_ref=%lf]\n", prop.A_ref);
			}
			//-----------------------
#endif			
		}
		...
		{
			prop.A_ref=propa.aed+propa.emd*prop.dist;
		
#ifdef DBG_ITWOM
			//-----------------------
			if(f_fMr==1)
			{
			fprintf(fMr, "lrprop : (else prop.dist>propa.dx)[prop.A_ref=%lf]\n", prop.A_ref);
			}
			//-----------------------
#endif			
		}
	...
	if(f_fMr==1)
	{
	fprintf(fMr, "lrprop : (prop.A_ref=mymax)[prop.A_ref=%lf]\n", prop.A_ref);
	}
	//-----------------------
#endif
}
...
void lrprop2(double d, prop_type &prop, propa_type &propa)
{
	...
	if (prop.mdp!=0) /* if oper. mode is not 0, i.e. not area mode ongoing */
  	{
		...
		propa.d_Ls=propa.dls[0]+propa.dls[1];
		propa.d_Ls=mymin(propa.d_Ls,1000000.0);
		propa.dla=prop.d_Lj[0]+prop.d_Lj[1];	
		propa.tha=mymax(prop.the[0]+prop.the[1],-propa.dla*prop.gamma_e);		
		wlos=false;
		wscat=false;
		...
		/*for (j=0; j<2; j++)
		     if (prop.d_Lj[j]<0.1*propa.dls[j] || prop.d_Lj[j]>3.0*propa.dls[j])
				prop.kwx=mymax(prop.kwx,3);  */
		...
		xae=pow(prop.wn*(prop.gamma_e*prop.gamma_e),-THIRD);  
		d3=mymax(propa.d_Ls,1.3787*xae+propa.dla);
		d4=d3+2.7574*xae;
	...
	if (prop.dist<propa.d_Ls)
	{
		...
		{		
			...
			{
				prop.A_ref=alos2(pd1,prop,propa);
			}
			else
			{			
			 	if (int(prop.dist-prop.d_Lj[0])==0)  /* if at 1st horiz */
				{
				prop.A_ref=5.8+alos2(pd1,prop,propa);
				}
				else if (int(prop.dist-prop.d_Lj[0])>0.0)    /* if past 1st horiz */
				{
				q=adiff2(0.0,prop,propa);
				prop.A_ref=adiff2(pd1,prop,propa);
				}			
				else
				{
				prop.A_ref=1.0;
...
double AttenuationForTerrainShielding(double zzt, double zzl, double zzc, prop_type &prop, propv_type &propv)
{
	static	int kdv;
	...
	int temp_klim=propv.radio_climate-1;
	...
	if (propv.lvar>0)
	{
		...
		switch (propv.lvar)
		{
			...
			if (propv.radio_climate<=0 || propv.radio_climate>7)
			{
				propv.radio_climate=5;
				temp_klim=4;
			...
			dexa=sqrt(18e6*prop.h_e[0])+sqrt(18e6*prop.h_e[1])+pow((575.7e12/prop.wn),THIRD);
			// fall throught
		...
		if(f_fMr==1)
		{
		fprintf(fMr, "AttenuationForTerrainShielding : [curve vmd=%lf]\n",  vmd);
		fprintf(fMr, "AttenuationForTerrainShielding : [curve sgtm=%lf]\n", sgtm);
		fprintf(fMr, "AttenuationForTerrainShielding : [curve sgtp=%lf]\n", sgtp);
		}
		//-----------------------
#endif
		...
		if(f_fMr==1)
		{
		fprintf(fMr, "AttenuationForTerrainShielding : [sgtd=%lf]\n", sgtd);
		fprintf(fMr, "AttenuationForTerrainShielding : [tgtd=%lf]\n", tgtd);
		}
		//-----------------------
#endif
		...
		{
			// Alg [3.9]
			q=(1.0-0.8*exp(-prop.dist/50e3))*prop.delta_h*prop.wn;
			// [Alg 5.9]
			sgl=10.0*q/(q+13.0);
		}
		...
		if(f_fMr==1)
		{
		fprintf(fMr, "AttenuationForTerrainShielding : [sgl=%lf]\n", sgl);
		}
#endif
		...
		if(f_fMr==1)
		{
		fprintf(fMr, "AttenuationForTerrainShielding : [vs0=%lf]\n", vs0);
		}
		//-----------------------
#endif
	...
	if(f_fMr==1)
	{
	fprintf(fMr, "AttenuationForTerrainShielding : [prop.kwx=%i]\n", prop.kwx);
	}
	//-----------------------
#endif
	...
	if(f_fMr==1)
	{
	fprintf(fMr, "AttenuationForTerrainShielding : [sgt=%lf]\n", sgt);
	}
	//-----------------------
#endif
	...
	if(f_fMr==1)
	{
	fprintf(fMr, "AttenuationForTerrainShielding : [yr=%lf][propv.sgc=%lf]\n", yr, propv.sgc);
	}
	//-----------------------
#endif
	...
	if(f_fMr==1)
	{
	fprintf(fMr, "AttenuationForTerrainShielding : [prop.A_ref=%lf]\n", prop.A_ref);
	fprintf(fMr, "AttenuationForTerrainShielding : [vmd=%lf]\n", vmd);
	fprintf(fMr, "AttenuationForTerrainShielding : [yr=%lf]\n", yr);
	fprintf(fMr, "AttenuationForTerrainShielding : [propv.sgc=%lf]\n", propv.sgc);
	fprintf(fMr, "AttenuationForTerrainShielding : [zc=%lf]\n", zc);
	fprintf(fMr, "AttenuationForTerrainShielding : [avarv=%lf]\n", avarv);
	}
	//-----------------------
#endif
...
{	
	...
	prop.d_Lj[0]=prop.dist;
	prop.d_Lj[1]=prop.dist;
...
void hzns2(double pfl[], prop_type &prop, propa_type &propa)
{
		...
		{				
			for(i=1; i<np; i++)
			{
				...
				if(q>0.0)
				{
					...
					prop.hhr=pfl[np+2-i];					
					prop.d_Lj[1]=mymax(0.0,prop.dist-sb);
				}
			}
			prop.the[0]=atan((prop.hht-za)/prop.d_Lj[0])-0.5*prop.gamma_e*prop.d_Lj[0];
			prop.the[1]=atan((prop.hhr-zb)/prop.d_Lj[1])-0.5*prop.gamma_e*prop.d_Lj[1];
		}
	}

	if((prop.d_Lj[1])<(prop.dist))
	{
		dshh=prop.dist-prop.d_Lj[0]-prop.d_Lj[1];

		if(int(dshh)==0) /* one obstacle */
		{
			dr=prop.d_Lj[1]/(1+zb/prop.hht);		
		}
		else	/* two obstacles */
		{		
			dr=prop.d_Lj[1]/(1+zb/prop.hhr);		
		}	
	}
	else    /* line of sight  */
	{
	...
	//////////////////////////////////////
	// mr modify 13/06/2016
	// added to correct problem memory pointer
	if((rp<0)||(rp>np))
	{
		rp=0;
	}
...
void qlrpfl(double pfl[], int klimx, int mdvarx, prop_type &prop, propa_type &propa, propv_type &propv)
{
	...
	xl[1]=prop.dist-xl[1];
	prop.delta_h=d1thx(pfl,xl[0],xl[1]);

	if (prop.d_Lj[0]+prop.d_Lj[1]>1.5*prop.dist)
	{
		...
		if (q<=prop.dist) /* if there is a rounded horizon, or two obstructions, in the path */
		{
			...
			for (j=0; j<2; j++)
			{
				prop.h_e[j]*=q; /* tx effective height set to be path dist/distance between obstacles */
				prop.d_Lj[j]=sqrt(2.0*prop.h_e[j]/prop.gamma_e)*exp(-0.07*sqrt(prop.delta_h/mymax(prop.h_e[j],5.0)));
			}
		}
	...
	else
	{
		...
		zlsq1(pfl,prop.dist-0.9*prop.d_Lj[1],xl[1],q,zb);
		prop.h_e[0]=prop.h_g[0]+FORTRAN_DIM(pfl[2],za);
		prop.h_e[1]=prop.h_g[1]+FORTRAN_DIM(pfl[np+2],zb);
	}
...
void qlrpfl2(double pfl[], int klimx, int mdvarx, prop_type &prop, propa_type &propa, propv_type &propv)
{
	...
	dlb=prop.d_Lj[0]+prop.d_Lj[1];
	prop.rch[0]=prop.h_g[0]+pfl[2];
	prop.rch[1]=prop.h_g[1]+pfl[np+2];

	for (j=0; j<2; j++)
		xl[j]=mymin(15.0*prop.h_g[j],0.1*prop.d_Lj[j]);

	xl[1]=prop.dist-xl[1];
	prop.delta_h=d1thx2(pfl,xl[0],xl[1],propa);
	...
	if ((np<1) || (pfl[1]>150.0))
	{
		...
		{	
			z1sq2(pfl,xl[0],0.9*prop.d_Lj[0],za,q);
			z1sq2(pfl,prop.dist-0.9*prop.d_Lj[1],xl[1],q,zb);
			prop.h_e[0]=prop.h_g[0]+FORTRAN_DIM(pfl[2],za);
			prop.h_e[1]=prop.h_g[1]+FORTRAN_DIM(pfl[np+2],zb);
		}
		...
		{			
			...
			if ((prop.d_Lj[0]+prop.d_Lj[1])<=prop.dist)
			{	
				/* q=pow(prop.dist/(dl[0]+dl[1])),2.0); */
				temp=prop.dist/(prop.d_Lj[0]+prop.d_Lj[1]);
				q=temp*temp;
			}
	...
	{
		prop.h_e[0]=prop.h_g[0]+(pfl[2]);
		prop.h_e[1]=prop.h_g[1]+(pfl[np+2]);
...
// for PROP_MODEL_LRICE
void point_to_point_ITM(
	double elev[], 
	double Tx_Alt,				// Transceiver above ground level
	double Rx_Alt,				// Receiver above groud level
	double eps_dielect,			// Earth dielectric constant (rel. permittivity)
	double sgm_conductivity,   	// Earth conductivity (Siemens/m)
	double AtmBendingCons,      // Atmospheric bending const, n-Units
	double frq_mhz, 
	int radio_climate,			// 1..7
	int polarization,           // 0 horizontal, 1 vertical
	double FractionSituations,  // 0.01 .. .99, Fractions of situations
	double FractionTime,        // 0.01 .. .99, Fractions of time
	double &path_loss_db, 
	char *strmode, 
	int &errnum
	)
{
	...
	double zsys=0;
	double zc, ztime;
	double atm_bend_const, enso, q;
	long ja, jb, i, num_of_points;
	/* double dkm, xkm; */
	double free_space_path_loss;

		
#ifdef DBG_ITWOM
	//------------------------
	f_fMr = 0;		// se 1 stampa abilita la stampa
	if(((int)elev[0])==18)
		f_fMr = 1;
	//------------------------
#endif
	...
	prop.h_g[0]=Tx_Alt;
	prop.h_g[1]=Rx_Alt;
	propv.radio_climate=radio_climate;
	prop.kwx=0;
	propv.lvar=5;
	prop.mdp=-1;
	zc=qerfi(FractionSituations);
	ztime=qerfi(FractionTime);
	num_of_points=(long)elev[0];
	/* dkm=(elev[1]*elev[0])/1000.0; */
	/* xkm=elev[1]/1000.0; */
	atm_bend_const=AtmBendingCons;
	enso=0.0;
	q=enso;
	...
	propv.mdvar=12;
	qlrps(frq_mhz,zsys,q,polarization,eps_dielect,sgm_conductivity,prop);
	qlrpfl(elev,propv.radio_climate,propv.mdvar,prop,propa,propv);
	
	// https://en.wikipedia.org/wiki/Free-space_path_loss
	free_space_path_loss=32.45+20.0*log10(frq_mhz)+20.0*log10(prop.dist/1000.0);
	...
	else
	{
		...
		if (prop.dist<=propa.d_Ls || prop.dist <= propa.dx)
	...
	path_loss_db = AttenuationForTerrainShielding(ztime, 0.0, zc, prop, propv) + free_space_path_loss;
...
*****************************************************************************/
{
	...
	qlrpfl2(elev,propv.radio_climate,propv.mdvar,prop,propa,propv);
	tpd=sqrt((prop.h_e[0]-prop.h_e[1])*(prop.h_e[0]-prop.h_e[1])+(prop.dist)*(prop.dist));
	...
	dbloss=AttenuationForTerrainShielding(zr,0.0,zc,prop,propv)+fs;
	errnum=prop.kwx;
}
...
*************************************************************************************************/
{
  ...
  if(int(q)<0.0)
    ...
    { if(int(q)==0.0)
        propmode = 4;  // 1-Single Horizon
      else if(int(q)>0.0)
        propmode = 8;  // 2-Double Horizon
      if(prop.dist<=propa.d_Ls || prop.dist<=propa.dx)
        propmode += 1; // Diffraction Dominant
      else if(prop.dist>propa.dx)
        propmode += 2; // Troposcatter Dominant
    }
  dbloss = AttenuationForTerrainShielding(ztime, zloc, zconf, prop, propv) + fs;      //AttenuationForTerrainShielding(time,location,confidence)
  errnum = prop.kwx;
}
...
*************************************************************************************************/
{
  ...
  dbloss = AttenuationForTerrainShielding(zr,0.0,zc,prop,propv)+fs;      //AttenuationForTerrainShielding(time,location,confidence)
  errnum = prop.kwx;
}
...
double pctTime, double pctLoc, double pctConf, double &dbloss, char *strmode, int &errnum)
{
	...
	prop.h_g[0]=tht_m;
	prop.h_g[1]=rht_m;
	propv.radio_climate=(long)radio_climate;
	prop.encc=enc_ncc_clcref;
	prop.cch=clutter_height;
	prop.cd=clutter_density;
	prop.dhd=delta_h_diff;
	...
	qlra(kst, propv.radio_climate, ivar, prop, propv);
	...
	fs=32.45+20.0*log10(frq_mhz)+20.0*log10(prop.dist/1000.0);
	xlb=fs+AttenuationForTerrainShielding(zt, zl, zc, prop, propv);
	dbloss=xlb;
	if (prop.kwx==0)
		errnum=0;
	else
